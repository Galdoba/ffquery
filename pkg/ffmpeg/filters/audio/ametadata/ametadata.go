...
// Ametadata представляет видеофильтр metadata из FFmpeg.
...
// New создаёт фильтр Ametadata и применяет переданные опции.
...
func New(opts ...options.Option) *Ametadata {
	m := &Ametadata{}
...
func (m *Ametadata) apply(opt options.Option) error {
	switch opt.Key {
	case "mode":
...
func (m *Ametadata) Validate() error {
	if m.Mode == "" {
...
func (m *Ametadata) String() string {
	if m.Err() != nil {
		return ""
	}
	var parts []string
...
func (m *Ametadata) ProvideOption() options.Option {
	if m.Err() != nil {
